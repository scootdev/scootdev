...
function Navbar() {
  return (
    <AppBar position="static" sx={{ backgroundColor: '#1E1E1E', paddingX: 7, color: '#BFBFFD' }}>
      <Toolbar sx={{ display: 'flex', justifyContent: 'space-between' }}>
        <div sx={{ display: 'flex', alignItems: 'center' }}>
          <Typography variant="h6" component="div" sx={{ flexGrow: 1, marginLeft: 2 }}>
            <Link component={RouterLink} to="/" color="inherit" underline="none" sx={{ fontWeight: 200 }}>ScootDev</Link>
          </Typography>
        </div>
        <nav>
